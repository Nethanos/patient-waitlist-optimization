...
export function computeScore(patient, targetLocation) {
  ...
  const acceptedNorm =
    patient.acceptedOffers != null ? normalize(patient.acceptedOffers, 0, 100) : null;

  const canceledNorm =
    patient.canceledOffers != null ? 1 - normalize(patient.canceledOffers, 0, 100) : null;

  const replyTimeNorm =
    patient.averageReplyTime != null ? 1 - normalize(patient.averageReplyTime, 0, 3600) : null;
